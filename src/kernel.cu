...
__global__ void kernRearrange(
	...
	glm::vec3* rearrangedPos, glm::vec3* rearrangedVel) {
	...
	rearrangedVel[index] = vel[particleArrayIndices[index]];
}

//Reshuffle the vel buffers 
__global__ void kernPingPong(
	int N, int* particleArrayIndices,
	glm::vec3* vel2, glm::vec3* vel1) {
	int index = threadIdx.x + blockIdx.x * blockDim.x;
	if (index >= N)
		return;
	vel1[particleArrayIndices[index]] = vel2[index];
}
...
__global__ void kernUpdateVelNeighborSearchCoherent(
	...
	//neighboring distance to check for boids 
	float neighborDist = imax(imax(rule1Distance, rule2Distance), rule3Distance);
	glm::ivec3 maxIndex = ((pos[index] - gridMin) + neighborDist) * inverseCellWidth;
	glm::ivec3 minIndex = ((pos[index] - gridMin) - neighborDist) * inverseCellWidth;
